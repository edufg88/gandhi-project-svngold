class GPLForeArmArmorItem extends GPArmorItem;

simulated function AdjustPawn(GPPlayerPawn P, bool bRemoveBonus)
{
	if (P != None)
	{		
		AttachToPawn(P, P.LowerLeftArmArmorSocketName);
	}
}
...
function RenderStats(HUD HUD)
{
	...
	HUD.Canvas.DrawText("L ForeArm Armor");
}

function string GetName()
{
	return "Left Forearm Armor";
}

defaultproperties
{
	Begin Object Class=StaticMeshComponent Name=StaticMeshComponent1
		StaticMesh=StaticMesh'Gandhi.Mesh.SM_armor_Larm_02'
		bOnlyOwnerSee=false
		CastShadow=false
