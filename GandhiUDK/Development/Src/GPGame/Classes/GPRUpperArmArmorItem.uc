class GPRUpperArmArmorItem extends GPArmorItem;

simulated function AdjustPawn(GPPlayerPawn P, bool bRemoveBonus)
{
	if (P != None)
	{		
		AttachToPawn(P, P.UpperRightArmArmorSocketName);
	}
}
...
function RenderStats(HUD HUD)
{
	...
	while(i < NbCases && i < 10)
	{
		...
		HUD.Canvas.SetDrawColor(10,10,250,200); //R,G,B
	...
	HUD.Canvas.SetDrawColor(10, 10, 250, 200);
	HUD.Canvas.Font = class'Engine'.static.GetSmallFont();
	HUD.Canvas.DrawText("R UpperArm Armor");
}

function string GetName()
{
	return "Right Upper Arm Armor";
}
...
defaultproperties
{
	Begin Object Class=StaticMeshComponent Name=StaticMeshComponent1
		StaticMesh=StaticMesh'Gandhi.Mesh.SM_armor_Rarm_01'
		bOnlyOwnerSee=false
		CastShadow=false
