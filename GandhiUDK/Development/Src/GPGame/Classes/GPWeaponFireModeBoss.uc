class GPWeaponFireModeBoss extends Object
...
var ProtectedWrite GPWeaponBoss Owner;
...
simulated function SetOwner(GPWeaponBoss NewOwner)
{
	if (NewOwner != None)
	{
		Owner = NewOwner;
...
simulated function PlayFiringEffects(optional Vector HitLocation)
{
	...
	if (SkeletalMeshComponent != None && SkeletalMeshComponent.GetSocketByName(Owner.Boss.WeaponSocketName) != None)
	{
		...
		if (FireParticleTemplate != None)
		{
			...
			if (MuzzleFlashParticleSystem == None)
			{
				...
				if (MuzzleFlashParticleSystem != None)
				{
					...
					SkeletalMeshComponent.AttachComponentToSocket(MuzzleFlashParticleSystem, Owner.Boss.WeaponSocketName);
				}
			}
		...
		if (FireLight != None)
		{
			...
			{
				SkeletalMeshComponent.AttachComponentToSocket(FireLight, Owner.Boss.WeaponSocketName);
