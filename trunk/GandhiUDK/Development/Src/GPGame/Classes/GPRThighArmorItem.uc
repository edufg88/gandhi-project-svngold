class GPRThighArmorItem extends GPArmorItem;

simulated function AdjustPawn(GPPlayerPawn P, bool bRemoveBonus)
{
	if (P != None)
	{		
		AttachToPawn(P, P.UpperRightLegArmorSocketName);
	}
}
...
function RenderStats(HUD HUD)
{
	...
	HUD.Canvas.DrawText("R Thigh Armor");
}

function string GetName()
{
	return "Right Thigh Armor";
}

defaultproperties
{
	Begin Object Class=StaticMeshComponent Name=StaticMeshComponent1
		StaticMesh=StaticMesh'Gandhi.Mesh.SM_armor_Rleg_01'
		bOnlyOwnerSee=false
		CastShadow=false
