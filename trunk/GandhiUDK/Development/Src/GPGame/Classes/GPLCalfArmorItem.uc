class GPLCalfArmorItem extends GPArmorItem;

simulated function AdjustPawn(GPPlayerPawn P, bool bRemoveBonus)
{
	if (P != None)
	{		
		AttachToPawn(P, P.LowerLeftLegArmorSocketName);
	}
}
...
function RenderStats(HUD HUD)
{
	...
	HUD.Canvas.DrawText("L Calf Armor");
}

function string GetName()
{
	return "Left Calf Armor";
}

defaultproperties
{
	Begin Object Class=StaticMeshComponent Name=StaticMeshComponent1
		StaticMesh=StaticMesh'Gandhi.Mesh.SM_armor_Lleg_02'
		bOnlyOwnerSee=false
		CastShadow=false
