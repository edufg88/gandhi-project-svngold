class GPRForeArmArmorItem extends GPArmorItem;

simulated function AdjustPawn(GPPlayerPawn P, bool bRemoveBonus)
{
	if (P != None)
	{		
		AttachToPawn(P, P.LowerRightArmArmorSocketName);
	}
}
...
function RenderStats(HUD HUD)
{
	...
	HUD.Canvas.DrawText("R ForeArm Armor");
}

function string GetName()
{
	return "Right Forearm Armor";
}

defaultproperties
{
	Begin Object Class=StaticMeshComponent Name=StaticMeshComponent1
		StaticMesh=StaticMesh'Gandhi.Mesh.SM_armor_Rarm_02'
		bOnlyOwnerSee=false
		CastShadow=false
