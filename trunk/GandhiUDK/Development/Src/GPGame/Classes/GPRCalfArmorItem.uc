class GPRCalfArmorItem extends GPArmorItem;

simulated function AdjustPawn(GPPlayerPawn P, bool bRemoveBonus)
{
	if (P != None)
	{		
		AttachToPawn(P, P.LowerRightLegArmorSocketName);
	}
}
...
function RenderStats(HUD HUD)
{
	...
	HUD.Canvas.DrawText("R Calf Armor");
}

function string GetName()
{
	return "Right Calf Armor";
}

defaultproperties
{
	Begin Object Class=StaticMeshComponent Name=StaticMeshComponent1
		StaticMesh=StaticMesh'Gandhi.Mesh.SM_armor_Rleg_02'
		bOnlyOwnerSee=false
		CastShadow=false
