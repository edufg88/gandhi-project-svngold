class GPWeaponFireModeSiri extends Object
...
var ProtectedWrite GPWeaponSiri Owner;
...
simulated function SetOwner(GPWeaponSiri NewOwner)
{
	if (NewOwner != None)
	{
		Owner = NewOwner;
...
simulated function PlayFiringEffects(optional Vector HitLocation)
{
	...
	SkeletalMeshComponent = Owner.Siri.Mesh;
	if (SkeletalMeshComponent != None && SkeletalMeshComponent.GetSocketByName(Owner.Siri.WeaponSocketName) != None)
	{
		...
		if (FireParticleTemplate != None)
		{
			...
			if (MuzzleFlashParticleSystem == None)
			{
				...
				if (MuzzleFlashParticleSystem != None)
				{
					...
					SkeletalMeshComponent.AttachComponentToSocket(MuzzleFlashParticleSystem, Owner.Siri.WeaponSocketName);
				}
			}
		...
		if (FireLight != None)
		{
			...
			{
				SkeletalMeshComponent.AttachComponentToSocket(FireLight, Owner.Siri.WeaponSocketName);
