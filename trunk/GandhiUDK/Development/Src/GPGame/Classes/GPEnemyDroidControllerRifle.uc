class GPEnemyDroidControllerRifle extends GPEnemyDroidController;
...
function WarnEnemies(Pawn PwnGandhi)
{
	...
	PZ = DroidPawn.PatrolZ;

	foreach PZ.Droids(P)
	{
		GPEnemyDroidControllerRifle(P.Controller).SetWarned(PwnGandhi);
	}
...
function WhatToDo()
{
	...
	if (CurrentEnemy == None || VSize(Gandhi.Location - Pawn.Location) > SightRadius)
	{
		...
		NextLoc = PatrolZone.PatrolPoints[DroidPawn.PZindex].Location;

		SetFocalPoint(NextLoc);
		// Set the path finding final destination
		NavigationHandle.SetFinalDestination(NextLoc);
		// Set the current enemy as the destination
		SetDestinationPosition(NextLoc);
